...
- **Seed color:** `#1A56DB` (used to derive `ColorScheme` via `ColorScheme.fromSeed` where applicable).
...
Primary family: **Inter VariableFont Opsz,Wght**

- **Inter VariableFont Opsz,Wght** — 1 file(s) configured in `pubspec.yaml` (app-wide `fontFamily` when primary)
...
Import tokens via `package:gnade_app/src/theme/theme_constants.dart`.
